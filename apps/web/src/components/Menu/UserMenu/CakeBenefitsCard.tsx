...
const CakeBenefitsCardWrapper = styled(Box)`
  ...
  background: linear-gradient(180deg, #53dee9, #45d9be);
...
const CakeBenefitsCard: React.FC<React.PropsWithChildren<CakeBenefitsCardProps>> = ({ onDismiss }) => {
  ...
  const {
    ...
    <>
      <Text>
        {t(`%lockedCake% DEF (including rewards) are locked in the DEF Pool until %lockedEndTime%`, {
  ...
  const {
    ...
    <>
      <Text>
        {t(`iCAKE allows you to participate in the IFO public sales and commit up to %iCake% amount of DEF.`, {
        ...
        <>
          ...
          <Message mt="8px" mb="16px" variant="warning">
            <MessageText maxWidth="200px">
              {t(
                'Lock DEF to enjoy the benefits of farm yield boosting, participating in IFOs, voting power boosts, and so much more!',
              )}{' '}
              <NextLinkFromReactRouter
